...
if len(sys.argv) < 2:
    sys.stderr.write('Usage: fusion2017.py NUM_DEPARTEMENT ')
    sys.exit(1)

with open('fusion2017.json') as json_file:
    ...
    for fusion in fusions:
        if fusion['insee'][0:len(sys.argv[1])]==sys.argv[1]:
            ...
            overpass = """http://overpass-api.de/api/interpreter?data=[out:json];relation["start_date"~"^2017"]["admin_level"~"(|8)"][name~"^(%s)$",i];out;""" % (fusion['nom'].replace(' ','.').replace('-','.').replace('É','.'))
            osm = requests.get(overpass)
            ...
            if len(nouvelle_json['elements']) == 0:
              ...
              if insee == "":
                ...
                insee_tag = """<tag k='ref:INSEE' v='%s' /><tag k='fixme' v='ref:INSEE fixé arbitrairement à celui du chef-lieu, à vérifier avec COG2017' />""" % insee
        ...
        <osm version='0.6' upload='true' generator='fusion2016.py'>
          <relation id='-1' action='modify' visible='true'>
            ...
            %s
              ...
              tags = "start_date=2017-01-01|name="+fusion['nom']
